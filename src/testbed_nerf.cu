...
inline __host__ __device__ float calc_cone_angle(
	...
	return cone_angle_constant; // cone_angle_constant -- 0
}
...
__global__ void extract_srgb_with_activation(
	...
	ENerfActivation rgb_activation, bool from_linear
) {
...
__global__ void mark_untrained_density_grid(
	...
	for (uint32_t j = 0; j < n_training_images && count < min_count; ++j) {
		...
		for (uint32_t k = 0; k < 8; ++k) {
			...
			Vector2f uv = pos_to_uv(corners[k], m.resolution, m.focal_length,
				xform, m.principal_point, Vector3f::Zero(), {}, m.lens);
			...
			Ray ray = uv_to_ray(0.0f, uv, m.resolution, m.focal_length,
				xform, m.principal_point, Vector3f::Zero(), 0.0f, 1.0f, 0.0f, {}, {}, m.lens);
			if ((ray.d.normalized() - dir).norm() < 1e-3f && uv.x() > 0.0f 
				&& uv.y() > 0.0f && uv.x() < 1.0f && uv.y() < 1.0f) {
				++count;
				break;
			}
		}
...
__global__ void generate_grid_samples_nerf_uniform(
	...
	// render_aabb_to_local -- Matrix3f::Identity()	
	// render_aabb -- [min=[0,0,0], max=[1,1,1]]	
	pos = render_aabb_to_local.transpose() * (pos.cwiseProduct(render_aabb.max - render_aabb.min) + render_aabb.min);
	// train_aabb -- [min=[0,0,0], max=[1,1,1]]	
...
__global__ void decay_sharpness_grid_nerf(
	const uint32_t n_elements,
	float decay,
...
__global__ void advance_pos_nerf(
	const uint32_t n_elements,
	BoundingBox render_aabb, 	// render_aabb -- [min=[0,0,0], max=[1,1,1]]	
	Matrix3f render_aabb_to_local, 	// Matrix3f::Identity()
	Vector3f camera_fwd, // camera_matrix1.col(2)
	Vector2f focal_length,
	uint32_t sample_index, // sample_index -- 0
	NerfPayload* __restrict__ payloads,
	const uint8_t* __restrict__ density_grid,
	uint32_t min_mip,
	float cone_angle_constant // cone_angle_constant -- 0
) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
...
__global__ void generate_next_nerf_network_inputs(
	...
	BoundingBox render_aabb,	// render_aabb -- [min=[0,0,0], max=[1,1,1]]
	Matrix3f render_aabb_to_local,	// Matrix3f::Identity()
	BoundingBox train_aabb, // train_aabb -- [min=[0,0,0], max=[1,1,1]]
	Vector2f focal_length,
	Vector3f camera_fwd,
	NerfPayload* __restrict__ payloads,
	PitchedPtr<NerfCoordinate> network_input,
...
__global__ void composite_kernel_nerf(
	...
	BoundingBox aabb,
	float glow_y_cutoff, // glow_y_cutoff -- 0
	int glow_mode, // glow_mode -- 0
	Matrix<float, 3, 4> camera_matrix,
	Vector2f focal_length,
	float depth_scale, // depth_scale -- 3.0303
	...
	ENerfActivation rgb_activation,
	ENerfActivation density_activation,
	int show_accel, // show_accel == -1
	float min_transmittance // min_transmittance -- 0.01
) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	if (i >= n_elements) return;
	...
	for (; j < actual_n_steps; ++j) {
		...
		if (glow_mode) { // random grid visualizations ftw!
			...
			bool green_grid = glow_mode & 1; // glow_mode -- 0
		...
		if (render_mode == ERenderMode::Normals) {
			...
			rgb = Array3f::Constant(cam_fwd.dot(pos - origin) * depth_scale); // depth_scale -- 3.0303
		...
		if (local_rgba.w() > (1.0f - min_transmittance)) { // min_transmittance -- 0.01
...
inline __device__ Vector2f sample_cdf_2d(
	Vector2f sample, 
	uint32_t img, 
	const Vector2i& res, 
...
inline __device__ float pdf_2d(
	Vector2f sample, 
	uint32_t img, 
	const Vector2i& res, 
	const float* __restrict__ cdf_x_cond_y, 
...
inline __device__ Vector2f nerf_random_image_pos_training(
	...
	if (snap_to_pixel_centers) {  // snap_to_pixel_centers -- 0
...
__global__ void generate_training_samples_nerf(
	...
	bool snap_to_pixel_centers,  // snap_to_pixel_centers -- 0
	bool train_envmap,
	float cone_angle_constant, // cone_angle_constant -- 0
	...
	// ===>
	// ray_unnormalized: o.x = 1.63, o.y = 1.10, o.z = 0.87
	// ray_unnormalized: o.x = 1.63, o.y = 1.10, o.z = 0.87
...
__global__ void compute_loss_kernel_train_nerf(
	...
	if (compacted_numsteps == numsteps && envmap_gradient) {
		...
		if (!train_in_linear_colors) { // train_in_linear_colors -- 0
...
__global__ void init_rays_with_payload_kernel_nerf(
	uint32_t sample_index, // sample_index -- 0
	NerfPayload* __restrict__ payloads, // ==> output
	Vector2i resolution,
	Vector2f focal_length,
	Matrix<float, 3, 4> camera_matrix0,
	Matrix<float, 3, 4> camera_matrix1,
	Vector4f rolling_shutter,
	Vector2f screen_center,  // screen_center -- 0.5
	Vector3f parallax_shift, // parallax_shift -- 0
	bool snap_to_pixel_centers,  // snap_to_pixel_centers -- 0
	BoundingBox render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
	Matrix3f render_aabb_to_local, // Matrix3f::Identity()
	float near_distance,
	float plane_z, // plane_z -- 1.36364
	float aperture_size, // aperture_size -- 0
	Foveation foveation,
	Lens lens,
	Buffer2DView<const Eigen::Array4f> envmap,
	Array4f* __restrict__ frame_buffer,
	...
	Ray ray = uv_to_ray(
		...
		near_distance,
		plane_z, // plane_z -- 1.36364
		aperture_size, // aperture_size -- 0
		foveation,
		hidden_area_mask,
		lens,
		distortion
	...
	depth_buffer[idx] = MAX_DEPTH(); // 16384.0f
...
void Testbed::NerfTracer::init_rays_from_camera(
	...
	const Vector3f& parallax_shift, // parallax_shift -- 0
	bool snap_to_pixel_centers,  // snap_to_pixel_centers -- 0
	const BoundingBox& render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
	const Matrix3f& render_aabb_to_local, // Matrix3f::Identity()
	float near_distance,
	float plane_z, // plane_z -- 1.36364
	float aperture_size, // aperture_size -- 0
	...
	init_rays_with_payload_kernel_nerf<<<blocks, threads, 0, stream>>>(
		...
		screen_center, // screen_center -- 0.5
		parallax_shift, // parallax_shift -- 0
		snap_to_pixel_centers, // snap_to_pixel_centers -- 0
		render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
		render_aabb_to_local, // Matrix3f::Identity()
		near_distance,
		plane_z, // plane_z -- 1.36364
		aperture_size, // aperture_size -- 0
	...
	linear_kernel(advance_pos_nerf, 0, stream,
		...
		render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
		render_aabb_to_local, // Matrix3f::Identity()
		camera_matrix1.col(2), // 0.0698417 ...
		focal_length,
		sample_index, // sample_index -- 0
		m_rays[0].payload,
		grid,
...
uint32_t Testbed::NerfTracer::trace(
	...
	const BoundingBox& render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
	const Eigen::Matrix3f& render_aabb_to_local, // Matrix3f::Identity()
	const BoundingBox& train_aabb, // train_aabb -- [min=[0,0,0], max=[1,1,1]]
	const Vector2f& focal_length,
	float cone_angle_constant, // cone_angle_constant -- 0
	const uint8_t* grid,
	ERenderMode render_mode,
	...
	while (i < MARCH_ITER) { // MARCH_ITER = 10000
		...
		uint32_t n_steps_between_compaction = tcnn::clamp(target_n_queries / n_alive, 
			(uint32_t)MIN_STEPS_INBETWEEN_COMPACTION, (uint32_t)MAX_STEPS_INBETWEEN_COMPACTION);
		...
		linear_kernel(generate_next_nerf_network_inputs, 0, stream,
			...
			render_aabb, // render_aabb -- [min=[0,0,0], max=[1,1,1]]
			render_aabb_to_local, // Matrix3f::Identity()
			train_aabb, // train_aabb -- [min=[0,0,0], max=[1,1,1]]
			focal_length,
			camera_matrix.col(2),
			rays_current.payload,
			input_data, // share with m_network_input
			n_steps_between_compaction, // n_steps_between_compaction -- 8
			grid,
			(show_accel>=0) ? show_accel : 0, // show_accel == -1
			cone_angle_constant, // cone_angle_constant -- 0
			extra_dims_gpu
		...
		// ====>
		// network_input = (pos, dir, dt ...)
		// payload.t = t;
		// payload.n_steps = n_steps;

		// n_steps_between_compaction -- 8
		...
		linear_kernel(composite_kernel_nerf, 0, stream,
			...
			i,
			train_aabb, // train_aabb -- [min=[0,0,0], max=[1,1,1]]
			glow_y_cutoff, // glow_y_cutoff -- 0
			glow_mode, // glow_mode -- 0
			camera_matrix,
			focal_length,
			depth_scale, // depth_scale -- 3.0303
			rays_current.rgba, // ==> output
			rays_current.depth, // ==> output
			rays_current.payload,
			input_data,	// network_input
			m_network_output, // share with rgbsigma_matrix
			...
			rgb_activation,
			density_activation,
			show_accel, // show_accel == -1
			min_transmittance // min_transmittance -- 0.01
		);
		// ====>
		// rgba[i] = local_rgba;
		// depth[i] = local_depth;
...
uint32_t Testbed::render_nerf(
	...
	tracer.init_rays_from_camera(
		...
		rolling_shutter,
		screen_center,  // screen_center -- 0.5
		m_parallax_shift, // parallax_shift -- 0
		m_snap_to_pixel_centers,
		m_render_aabb, // m_render_aabb -- [min=[0,0,0], max=[1,1,1]]
	...
	if (m_render_mode == ERenderMode::Slice) {
		...
		n_hit = tracer.trace(
			...
			m_render_aabb, // m_render_aabb -- [min=[0,0,0], max=[1,1,1]]
			m_render_aabb_to_local, // Matrix3f::Identity()
			m_aabb, // m_aabb -- [min=[0,0,0], max=[1,1,1]]
			focal_length,
			m_nerf.cone_angle_constant, // cone_angle_constant -- 0
			density_grid_bitfield,
			render_mode,
			camera_matrix1,
			depth_scale, // depth_scale -- 3.0303
			m_visualized_layer, // m_visualized_layer -- 0
			visualized_dimension, // visualized_dimension -- -1
			m_nerf.rgb_activation,
			m_nerf.density_activation,
			m_nerf.show_accel, // show_accel == -1
			m_nerf.render_min_transmittance, // render_min_transmittance -- 0.01
			m_nerf.glow_y_cutoff, // glow_y_cutoff -- 0
			m_nerf.glow_mode, // glow_mode -- 0
			extra_dims_gpu,
			stream
		); // ==> n_hit, rays_hit ...
	}
	...
	if (m_render_mode == ERenderMode::Slice) {
		...
		linear_kernel(generate_nerf_network_inputs_at_current_position, 0, stream, n_hit, 
...
void Testbed::load_nerf_post() { // moved the second half of load_nerf here
	...
	m_raw_aabb = m_aabb;
	m_render_aabb = m_aabb; // m_render_aabb -- [min=[0,0,0], max=[1,1,1]] ?
	m_render_aabb_to_local = m_nerf.training.dataset.render_aabb_to_local; // Matrix3f::Identity()
	// render_aabb -- [min=[0,0,0], max=[1,1,1]]	
...
void Testbed::train_nerf_step(uint32_t target_batch_size, Testbed::NerfCounters& counters, cudaStream_t stream) {
	...
	linear_kernel(generate_training_samples_nerf, 0, stream,
		...
		max_level,
		m_nerf.training.snap_to_pixel_centers,  // snap_to_pixel_centers -- 0
...
void Testbed::compute_mesh_vertex_colors() {
	...
	if (m_testbed_mode == ETestbedMode::Nerf) {
		...
		linear_kernel(extract_srgb_with_activation, 0, m_stream.get(), 
			n_verts * 3, 3, mlp_out.data(), (float*)m_mesh.vert_colors.data(), 
			m_nerf.rgb_activation, m_nerf.training.linear_colors);
			// m_nerf.training.linear_colors -- 0
	}
}
...
void assert_dataset_has_same_resolution(NerfDataset ds)
{
	// auto& ds = m_nerf.training.dataset;
	auto& res_0 = ds.metadata[0].resolution;

	for (int i = 1; i < ds.n_images; i++) {
		auto& res_i = ds.metadata[i].resolution;
		if (res_0 != res_i) {
			tlog::error() << "Image 0 resolution:" << res_0;
			tlog::error() << "Image " << i << " resolution:" << res_i;
			throw std::runtime_error{fmt::format("Resolution of dataset is not same.")};
		}
	}
}
...
TCNN_HOST_DEVICE void image_to_world(
	const uint32_t u,
	const uint32_t v,
	const float depth,
	const Eigen::Matrix3f RK_inverse,
	const Eigen::Matrix<float, 3, 4> camera_matrix,
	Eigen::Vector3f * __restrict__ X // world 
) {
	Vector3f uv_point = { depth * u, depth * v, depth };
	*X = RK_inverse * (uv_point - camera_matrix.col(3)); // R_inverse * K_inverse
}
...
TCNN_HOST_DEVICE float l2_float(
	Eigen::Vector3f a
) {
	return sqrtf(a.x() * a.x() + a.y()*a.y() + a.z() * a.z());
}

TCNN_HOST_DEVICE float get_disparity(
	const float focal_length,
	const Eigen::Matrix<float, 3, 4> camera_matrix1, 
	const Eigen::Matrix<float, 3, 4> camera_matrix2,
	const float depth
) {
	float baseline = l2_float(camera_matrix1.col(3) - camera_matrix2.col(3));
	// printf("focal_length: %.4f, baseline: %.4f, depth: %.4f\n",
...
__global__ void fusion_point_cloud_kernel(
	...
	const Vector2i resolution,
	const Vector2f focal_length,
	const Eigen::Matrix3f *RK_inverse, // K
	const TrainingXForm* training_xforms, 
	const Array4f *frame_buffer,
	const float *depth_buffer,
	NerfPointCloud * __restrict__ pc
) {
	uint32_t x = threadIdx.x + blockIdx.x * blockDim.x;
	uint32_t y = threadIdx.y + blockIdx.y * blockDim.y;
	if (x >= resolution.x() || y >= resolution.y())
	...
	uint32_t center = y * resolution.x() + x;
	uint32_t offset = image_k * resolution.y() * resolution.x()	+ center;
	float center_depth = depth_buffer[offset];
	// if ((x % 100 == 0) && (y % 100 == 0)) {
	// 	printf("center_depth: %.4f, offset = %d, image_k = %d\n", center_depth, offset, image_k);
	// }

	if (center_depth < 0.001f || center_depth >= MAX_DEPTH())
		return;

	Array4f center_color = frame_buffer[offset];
	Eigen::Vector3f X; // 3d point
	image_to_world(y, x, center_depth, RK_inverse[image_k], 
		training_xforms[image_k].start, &X);
	Eigen::Vector3f center_point = X;

	uint32_t count = 1; // include current point, color

	for (uint32_t image_j = 0; image_j < n_images && count < 6; image_j++) {
		if (image_j == image_k)
			continue;

		// Map world X to image j
		uint32_t temp_x, temp_y;
		float temp_depth;

		world_to_image(X, training_xforms[image_j].start, &temp_y, &temp_x, &temp_depth);
		if (temp_x >= resolution.x() || temp_y >= resolution.y())
			continue;
		offset = image_j * resolution.y() * resolution.x() + center;
		temp_depth = depth_buffer[offset];
		if (temp_depth < 0.001f || temp_depth >= MAX_DEPTH())
			continue;

		float center_disp = get_disparity(
							focal_length.x(),
							training_xforms[image_k].start,
							training_xforms[image_j].start,
							center_depth);
		float temp_disp = get_disparity(
							focal_length.x(),
							training_xforms[image_k].start,
							training_xforms[image_j].start,
							temp_depth);
		// printf("center_disp -- %.4f, temp_disp -- %.4f\n", center_disp, temp_disp);
		if (fabsf(center_disp - temp_disp) < 0.01f) {
			image_to_world(temp_y, temp_x, temp_depth, RK_inverse[image_j], 
				training_xforms[image_j].start, &X);
			center_point += X;
			center_color += frame_buffer[offset];
			count++;
		}
	}
	if (count >= 3) {
		// if ((x % 10) == 0 && (y % 10 == 0))
		// 	printf("1 --- count = %d, color:%.2f,%.2f,%.2f,%.2f\n", count,
		// 		center_color.x(), center_color.y(), center_color.z(), center_color.w());
		center_point *= (1.0f/count);
		center_color *= (1.0f/count);
		// if ((x % 10) == 0 && (y % 10 == 0))
		// 	printf("2 --- count = %d, color:%.2f,%.2f,%.2f,%.2f\n", count,
		// 		center_color.x(), center_color.y(), center_color.z(), center_color.w());

		pc[center].pos = center_point;
		pc[center].rgba = Eigen::Array4f {
			 linear_to_srgb(center_color.x()) * 255.0f,
			 linear_to_srgb(center_color.y()) * 255.0f,
			 linear_to_srgb(center_color.z()) * 255.0f,
			 linear_to_srgb(center_color.w()) * 255.0f
			};
	}
	// printf("count = %d\n", count);
}
...
std::vector<NerfPointCloud> Testbed::get_nerf_point_cloud() {
	...
	// Create RK_inverse
	GPUMemory<Eigen::Matrix3f> gpu_RK_inverse(ds.n_images);
	std::vector<Eigen::Matrix3f> cpu_RK_inverse;
	for (int image_k = 0; image_k < ds.n_images; image_k++) {
		auto& m = ds.metadata[image_k];
		const Vector2i resolution = m.resolution;
		...
		cpu_RK_inverse.push_back(R_inverse * K_inverse);
	}
	...
	GPUMemory<NerfPointCloud> one_image_gpu_points;
	std::vector<NerfPointCloud> one_image_cpu_points;
	...
	for (int image_k = 0; image_k < ds.n_images; image_k++) {
		...
		one_image_cpu_points.resize(n_elements);
		...
		fusion_point_cloud_kernel<<<blocks, threads, 0, m_stream.get()>>>(
			image_k,
			ds.n_images,
			m.resolution,
			m.focal_length,
		...
		for (int j = 0; j < one_image_cpu_points.size(); j++) {
			NerfPointCloud pc = one_image_cpu_points[j];
			Eigen::Vector3f pos = pc.pos.array().abs();
			if (pos.x() + pos.y() + pos.z() > 0.001f)
